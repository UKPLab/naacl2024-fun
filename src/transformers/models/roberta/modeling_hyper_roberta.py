...
topo_vecs = pickle.load(open("/storage/ukp/work/liu/data/topo_vec/lang_vecs.pkl", 'rb'))
lang2iso = {
"en": "eng", "ar":"arb",
"jv": "jav", 
"sw":"swa", "is":"isl", 
...
"id":"ind",
 "que":"que", 
"ta":"tam",
"th":"tha", "tr":"tur","vi":"vie","ru":"rus","ro":"ron", "es":'spa', 
"de":'deu', "hi":"hin", "el":"ell","zh": 'zho'}
...
class HyperLinear(nn.Module):
    ...
    """
    def __init__(self, base_model_hidden, input_emb_size, num_classes=2, hidden_dim=None):
        super().__init__()
        self.in_dim = input_emb_size
        if not hidden_dim:
            hidden_dim = input_emb_size
        self.hidden_dim = hidden_dim
        self.out_common = hidden_dim
        ...
        self.base_model_hidden = base_model_hidden
    ...
    def forward(self, data):
        ...
        weight = weight.view(batch_size, -1, self.base_model_hidden, self.num_classes)
...
class RobertaModel(BertModelAdaptersMixin, RobertaPreTrainedModel):
    ...
    def forward(
        ...
        outputs_embeds: Optional[bool] = None,
        ...
        if not return_dict and not outputs_embeds:
            return (sequence_output, pooled_output) + encoder_outputs[1:]
        if not return_dict and outputs_embeds:
            #return (sequence_output, pooled_output)+ encoder_outputs[1:],  embedding_output.detach()       
            return (sequence_output, pooled_output)+ encoder_outputs[1:],  torch.mean(embedding_output, dim=1).detach()       
        ...
        return BaseModelOutputWithPoolingAndCrossAttentions(
            ...
            embedding=embedding_output.detach(),
            #embedding=torch.mean(embedding_output, dim=1).detach(), #embedding_output.detach(),
        )
...
class RobertaForSequenceClassification(BertModelWithHeadsAdaptersMixin, RobertaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.hyper_layer = HyperLinear(config.hidden_size, config.hidden_size, self.num_labels)
    ...
    def forward(
        ...
        sequence_output = outputs[0]
        weights, bias = self.hyper_layer(sequence_output)
        print("weights", sequence_output.shape, weights.shape, bias.shape)
        temp = torch.einsum('bie,biec->bic', sequence_output, weights)
        logits = temp + bias
...
class RobertaForMultipleChoice(BertModelWithHeadsAdaptersMixin, RobertaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.hyper_layer = HyperLinear(config.hidden_size, config.hidden_size*2, 1, hidden_dim=int(config.hidden_size/4))
        # Initialize weights and apply final processing
        self.post_init()
    ...
    def forward(
        ...
        output_embs = outputs.embedding
        pooled_output = self.dropout(pooled_output)
        concat_output = torch.cat([pooled_output, output_embs], dim=1)
        weights, bias = self.hyper_layer(concat_output)
        temp = torch.einsum('bi,bic->bc', pooled_output, torch.squeeze(weights, dim=1))
        logits = temp + bias
...
class RobertaForTokenClassification(BertModelWithHeadsAdaptersMixin, RobertaPreTrainedModel):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        lang = lang2iso.get(config.lang)        
        self.lang_vec = torch.unsqueeze(torch.tensor(topo_vecs.get(lang)).float(), dim=0).to("cuda")
        ...
        self.hyper_layer = HyperLinear(config.hidden_size, config.hidden_size*2, config.num_labels, hidden_dim=int(config.hidden_size/8))
        #self.hyper_layer = HyperLinear(config.hidden_size, 289, config.num_labels, hidden_dim=int(config.hidden_size/8))
        self.lang_vec_proj = nn.Sequential(
         nn.Linear(289, 289),
         nn.ReLU(),
         nn.Linear(289, 32),
         nn.ReLU()
    ...
    def forward(
        ...
        #output_embs = self.lang_vec.repeat(sequence_output.shape[0], sequence_output.shape[1], 1).detach()
        #concat_output = output_embs
        output_embs = outputs.embedding
        concat_output = torch.cat([sequence_output, output_embs], dim=-1)
        weights, bias = self.hyper_layer(concat_output)
        #weights, bias = self.hyper_layer(sequence_output)
        temp = torch.einsum('bie,biec->bic', sequence_output, weights)
        logits = temp + bias
...
class RobertaForQuestionAnswering(BertModelWithHeadsAdaptersMixin, RobertaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.num_labels = config.num_labels
        self.roberta = RobertaModel(config, add_pooling_layer=False)
        # self.hyper_layer = HyperLinear(config.hidden_size, config.hidden_size, config.num_labels)
        self.hyper_layer = HyperLinear(config.hidden_size, config.hidden_size*2, config.num_labels,
    ...
    def forward(
        ...
        sequence_output = outputs[0]
        output_embs = outputs.embedding
        #output_embs = output_embs.unsqueeze(1).repeat(1, sequence_output.shape[1], 1)
        concat_output = torch.cat([sequence_output, output_embs], dim=-1)
        weights, bias = self.hyper_layer(concat_output)
        # weights, bias = self.hyper_layer(sequence_output)
        temp = torch.einsum('bie,biec->bic', sequence_output, weights)
